...
run-code: func [
  ...
  input [series!]
  /exit-on-output
  /local memory ip opcode inst modes params p1 p2 p3 out
][
  output: copy []
  memory: head ip: copy data
  ; opcode=(modes*100+inst) params=[p1 p2 p3]
  while [not tail? ip] [
    ; params [p1 p2 p3] = original parameters
    ; registers [r1 r2 r3] = modified parameters
    opcode: first+ ip

REBOL [
    Title: "Advent of Code 2019 - Day 02"
    Author: "Gaetan Barel"
    Version: 1.0.0
...
get-code: func [
    program [series!] "program to be read from"
    pos [integer!] "index to access [0-N]"
    /ptr "use the index stored at pos"
] [
    ; remap index from [0-N] to [1-N] (index can be out of bound!)
    index: pos + 1
    if (not none? ptr) and (index < length? program) [
        index: (pick program index) + 1
    ]
    ; print rejoin ["get-code [" :program "] " :pos " -> idx=" :index]
    pick program index
]
...
set-code: func [
    ...
    ; remap index from [0-N] to [1-N]
    index: pos + 1
    ; print rejoin ["set-code [" :program "] " :pos " " :val " -> idx=" :index]
    change/part (at program index) val 1
    program
]

step-code: func [
    "process the intcode at the given position"
    program [block!] "the list of opcodes"
    pos [integer!] "the position of the opcode"
] [
    op: get-code program pos
    w1: get-code/ptr program (pos + 1)
    w2: get-code/ptr program (pos + 2)
    w3: get-code program (pos + 3)
    ; probe rejoin [op " " w1 " " w2 " "w3]
    switch op [
        1 [ set-code program w3 (w1 + w2) ]
        2 [ set-code program w3 (w1 * w2) ]
    ]
    op
]

run-code: func [
    "process the intcode until code 99 is reached"
    program [block!] "the list of opcodes"
] [
    pos: 0
    until [
        op: step-code program pos
        pos: pos + 4
        op = 99
    ]
    get-code program 0
...
; 1. parse input
input-file: %2019-12-02-input.txt
; data: load replace/all read filepath "," " "
data: map-each entry parse read input-file "," [to-integer entry]

; 1.1 correct input values
data-part1: copy data
set-code data-part1 1 12 ; nound=12
set-code data-part1 2 2  ; verb=2

; 2. run code on loaded data
prin "Part 1: " print run-code data-part1
...
use [inputs] [
    ...
    for noun 0 99 1 [
        for verb 0 99 1 [
            data-temp: copy data
            set-code data-temp 1 noun
            set-code data-temp 2 verb
            output: run-code data-temp            
            if (output = 19690720) [
                append inputs ( noun * 100 + verb )
            ]
        ]
    ]
    prin "Answer part 2: "

REBOL [
  Title: "Advent of Code 2019 - Day 06"
  Author: "Gaetan Barel"
  Version: 1.0.0
...
data: read/lines %2019-12-06-input.txt
data2: ["COM)B" "B)C" "C)D" "D)E" "E)F" "B)G" "G)H" "D)I" "E)J" "J)K" "K)L"]

make_orbit_tree: func [data [series!]] [
  orbits: copy []
  foreach orbit data [
    parse orbit [
      copy parent to ")" 
    ...
    ; if parent not listed, add it
    any [
      (find orbits parent) 
      (repend orbits [parent (copy [])])
    ]
    ; if child listed, set parent, otherwise append child to orbits
    any [
      (find orbits child) 
      (repend orbits [child (copy [])])
    ]
    ; there should only ever be one parent!
    append second find orbits child parent
  ]
  orbits
]

count_orbits: func [orbits [series!] p /local count] [
  count: 0
  while [not empty? (p: second find head orbits p)] [
    count: (count + 1)
  ]
  count
]

count_all_orbits: func [data [series!] /local orbits count] [
  orbits: make_orbit_tree data
  ; go through all nodes and count their depth
  total_count: 0
  forskip orbits 2 [
    count: count_orbits orbits first orbits
...
data3: ["COM)B" "B)C" "C)D" "D)E" "E)F" "B)G" 
  "G)H" "D)I" "E)J" "J)K" "K)L" "K)YOU" "I)SAN"]

list_parents: func [orbits [series!] child [string!]] [
  parents: copy []
  p: child
  while [not empty? (p: second find orbits p)] [insert head parents p]
]

count_santa_orbits: func [data [series!] /local orbits] [
  orbits: make_orbit_tree data
  p1: list_parents orbits "SAN"
  p2: list_parents orbits "YOU"
  parent: last intersect p1 p2
  count_p1: (count_orbits orbits "SAN") - 1
  count_p2: (count_orbits orbits "YOU") - 1
  count_parent: (count_orbits orbits parent)
  (count_p1 + count_p2) - (2 * count_parent)
]

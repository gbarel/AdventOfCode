REBOL [
    Title: "Advent of Code 2018 - Day 02"
    Author: "Gaetan Barel"
    Version: 1.0.0
...
countletters: func [
	"Count the number of occurrence of all letters in a given word"
	str [string!] "the input word"
][
	results: array/initial 26 0
	foreach char str [
		index: 1 + (to-integer char) - (to-integer #"a")
...
makehash: func [
	"Returns the hash based on the number of words which have exactly 2 or 3 times the same letter"
	wordlist [block!] "the list of words"
][
	exactly2: 0
	exactly3: 0
	foreach word wordlist [
		ret: countletters word
		if find ret 2 [exactly2: exactly2 + 1]
		if find ret 3 [exactly3: exactly3 + 1  ]
	]
	exactly3 * exactly2
]

; read data
input-file: %2018-12-02-input.txt
